    ...
    public Broadcast updateServerModel(ServerModel model) {
        if (ServerModel.isValidName(newNickname)) {
            ...
            if (users.contains(newNickname)) {
                ...
                Collection<String> recipients = model.getSameChannelUsers(getSenderId());
    ...
    public Broadcast updateServerModel(ServerModel model) {
        if (ServerModel.isValidName(channel)) {
            ...
            if (channels.contains(channel)) {
                ...
                model.addChannel(getSenderId(), channel, inviteOnly);
                Collection<String> recipient = new TreeSet<>();
                recipient.add(getSender());
                return Broadcast.okay(this, recipient);
            }
        } else {
            return Broadcast.error(this, ServerResponse.INVALID_NAME);
        }
    ...
    public Broadcast updateServerModel(ServerModel model) {
        if ((model.getChannels()).contains(channel)) {
            if (model.isInviteOnly(channel)) {
                ...
                model.joinChannel(getSenderId(), channel);
                Collection<String> recipients = model.getUsersInChannel(channel);
                String owner = model.getOwner(channel);
...
class MessageCommand extends Command {
    ...
    public Broadcast updateServerModel(ServerModel model) {
        if ((model.getChannels()).contains(channel)) {
            if ((model.getUsersInChannel(channel)).contains(getSender())) {
                Collection<String> recipients = model.getUsersInChannel(channel);
                return Broadcast.okay(this, recipients);
            } else {
...
class LeaveCommand extends Command {
    ...
    public Broadcast updateServerModel(ServerModel model) {
        if ((model.getChannels()).contains(channel)) {
            if ((model.getUsersInChannel(channel)).contains(getSender())) {
                Collection<String> recipients = model.getUsersInChannel(channel);
                model.leaveChannel(getSenderId(), channel);
                return Broadcast.okay(this, recipients);
            } else {
                return Broadcast.error(this, ServerResponse.USER_NOT_IN_CHANNEL);
            }
    ...
    public Broadcast updateServerModel(ServerModel model) {
        if ((model.getChannels()).contains(channel)) {
            if (model.isInviteOnly(channel)) {
                ...
                } else if (!((model.getRegisteredUsers()).contains(userToInvite))) {
                    return Broadcast.error(this, ServerResponse.NO_SUCH_USER);
                } else {
                    model.joinChannel(model.getUserId(userToInvite), channel);
                    Collection<String> recipients = model.getUsersInChannel(channel);
                    return Broadcast.names(this, recipients, getSender());
                }
            } else {
        ...
        if ((model.getChannels()).contains(channel)) {
            if ((model.getRegisteredUsers()).contains(userToKick)) {
                if (!((model.getOwner(channel)).equals(getSender()))) {
                    ...
                    Collection<String> recipients = model.getUsersInChannel(channel);
                    model.leaveChannel(model.getUserId(userToKick), channel);
                    return Broadcast.okay(this, recipients);
                }
            } else {
                return Broadcast.error(this, ServerResponse.NO_SUCH_USER);
